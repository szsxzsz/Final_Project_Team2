...
<body>
	...
	<c:if test="${cardHistoryList != null }">
		<c:if test="${cardSum != null}">
		...
		<div class="container__tXwE">
			<ul class="cards_2AMVH">
				...
				<c:forEach var="cardHistoryVO" items="${cardHistoryList }">
				...
				<li>
					<div class="wrap_2lqJo" style="height: 150px">
						...
						<h4 class="title_3gORD">
							<strong> 
								<fmt:parseDate value="${cardHistoryVO.card_tran_date }" var="dateValue" pattern="yyyyMMdd"/>
								<fmt:formatDate value="${dateValue }" pattern="yyyy년 MM월 dd일"/>
							</strong>
						</h4>
						<section class="details_32db_">
							<img src="https://cdn-icons-png.flaticon.com/512/4329/4329479.png"
								class="landscapeImage_2tyu8 image_q-O13">
							<div class="desc_36Dy-">
								<h4 style="font-size: 2.5em; letter-spacing: 2px; margin: 10px 0 0 0">${cardHistoryVO.card_branch_name} </h4>
								<h4 style="font-size: 1.5em; letter-spacing: 2px; margin: 15px 0 0 5px">
									<fmt:parseDate value="${cardHistoryVO.card_tran_time}" var="timeValue" pattern="HHmmss"/>
									<fmt:formatDate value="${timeValue }" pattern="HH : mm"/>
								</h4>
							</div>
							<div class="buttons_2Q2OW" style="margin-top: -30px">
								<a class="detail_2z1En">
									<fmt:formatNumber value="${cardHistoryVO.card_tran_amt}"/> 원
								</a>
								<button class="issue_3gKkv">잔액 : <fmt:formatNumber value="${cardHistoryVO.after_card_amt}"/> 원</button>
							</div>	
						</section>
					</div>
				</li>
	...
	<c:if test="${empty cardHistoryList}">
